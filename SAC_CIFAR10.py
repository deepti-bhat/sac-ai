...
# Non-IID Data Loader for CIFAR-10
def get_non_iid_data_loader(batch_size=32, num_clients=10):
    ...
    data_per_client = len(cifar10_data) // num_clients
    client_loaders = []

    for client_id in range(num_clients):
        indices = list(range(client_id * data_per_client, (client_id + 1) * data_per_client))
        random.shuffle(indices)
        client_data = torch.utils.data.Subset(cifar10_data, indices)
...
def reptile_on_server(global_model, client_models, client_loaders, inner_steps=5, inner_lr=0.1):
...
def plot_and_print_metrics(train_accuracies, train_losses, test_accuracies, test_losses, conf_matrix, final_metrics):
    ...
    # Print final metrics
    accuracy, loss, f1, recall, precision = final_metrics
...
def main():
    ...
    test_loader = get_test_loader(batch_size=1000)
